    ...
    <table class="table table-hover">
      ...
      <tbody>
        <tr v-for="product in filteredProducts" :key="product.fin_prdt_cd">
          <td><RouterLink :to="{ name: 'savedetail', params: { fin_prdt_cd: product.fin_prdt_cd } }" class="link" :fin_prdt_cd="product.fin_prdt_cd">{{ product.dcls_strt_day }}</RouterLink></td>
          <td><RouterLink :to="{ name: 'savedetail', params: { fin_prdt_cd: product.fin_prdt_cd } }" class="link" :fin_prdt_cd="product.fin_prdt_cd">{{ product.kor_co_nm }}</RouterLink></td>
          <td><RouterLink :to="{ name: 'savedetail', params: { fin_prdt_cd: product.fin_prdt_cd } }" class="link" :fin_prdt_cd="product.fin_prdt_cd">{{ product.fin_prdt_nm }}</RouterLink></td>
          <td v-if="trm6"><RouterLink :to="{ name: 'savedetail', params: { fin_prdt_cd: product.fin_prdt_cd } }" class="link" :fin_prdt_cd="product.fin_prdt_cd">{{ product.intr_rate_6 }}</RouterLink></td>
          <td v-if="trm12"><RouterLink :to="{ name: 'savedetail', params: { fin_prdt_cd: product.fin_prdt_cd } }" class="link" :fin_prdt_cd="product.fin_prdt_cd">{{ product.intr_rate_12 }}</RouterLink></td>
          <td v-if="trm24"><RouterLink :to="{ name: 'savedetail', params: { fin_prdt_cd: product.fin_prdt_cd } }" class="link" :fin_prdt_cd="product.fin_prdt_cd">{{ product.intr_rate_24 }}</RouterLink></td>
          <td v-if="trm36"><RouterLink :to="{ name: 'savedetail', params: { fin_prdt_cd: product.fin_prdt_cd } }" class="link" :fin_prdt_cd="product.fin_prdt_cd">{{ product.intr_rate_36 }}</RouterLink></td>
        </tr>
      </tbody>
    </table>
  </div>
...
const uniqueCompanies = computed(() => {
  return [...new Set(store.saveOptions.map(product => product.kor_co_nm))];
});
...
const filteredProducts = computed(() => {
  if (selectedCompany.value === '') {
    ...
    return store.saveOptions.filter(product => product.kor_co_nm === selectedCompany.value);
  }
});
